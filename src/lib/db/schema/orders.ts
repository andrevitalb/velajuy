...
export const orders = pgTable(
	"orders",
	{
		...
		shippingZoneId: uuid("shipping_zone_id"), // FK to shipping_zones added in T6 (forward reference)
	...
	(t) => ({
		byUser: index("orders_user_idx").on(t.userId),
		byStatus: index("orders_status_idx").on(t.status),
		byPlacedAt: index("orders_placed_at_idx").on(t.placedAt),
	}),
)
...
export const orderItems = pgTable(
	...
	(t) => ({
		...
		byProduct: index("order_items_product_idx").on(t.productId),
	}),
)
